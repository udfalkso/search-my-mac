...
- An append-only float16 vector store with checksums, tombstones, atomic manifest publication, exact delta search, and the 10,000-vector/5% snapshot rebuild policy.

## Build and test

Requirements: macOS 13 or newer, Xcode command-line tools, Swift 6, and Rust/Cargo.
...
The checked-in application is a solid vertical slice, not yet a public-release build. SQLite FTS5 is the active in-process lexical backend; the bundled Tantivy library and authenticated XPC path are built and tested independently but still need to become the production data path. The Core ML E5 model, tokenizer, immutable USearch HNSW builder, hybrid RRF, native OOXML/Calamine adapters, file-handle-only extractor XPC service, provider-specific cloud tests, and the full million-record performance corpus are release-gate work.

Semantic and Hybrid controls currently report an effective text search rather than pretending semantic results are ready. Modern iWork uses the system importer; unsupported revisions remain filename-searchable.
